...
MV2_USE_CUDA=1 MV2_ENABLE_AFFINITY=0 MV2_SMP_USE_CMA=0 srun \
    --job-name=${JOBNAME} \
    --partition=${PARTITION} \
    --mpi=pmi2 --gres=gpu:8 -n${GPU_NUM} --ntasks-per-node=1 \
    --kill-on-bad-exit=1 \
    python /mnt/lustre/liukunlin/caad/alp/train2.py \
    --model_name="resnet_v2_50" \
    --hparams="train_adv_method=pgd_32_3_20,train_lp_weight=0.5" \
    --dataset="imagenet" \
    --dataset_image_size=299 \
    --output_dir="/mnt/lustre/liukunlin/caad/alp/pgd-image-299-2" \
    ...
    --finetune_checkpoint_path="/mnt/lustre/liukunlin/caad/alp/resnet_v2_50_2017_04_14" 2>&1 | tee log_${TIMESTAMP} &
