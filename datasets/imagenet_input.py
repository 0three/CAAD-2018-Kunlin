...
def my_func(image):
  ...
  a=np.array(a)
  for i in image:
    image_singer = wiener(i)
    a=np.append(a,image_singer)
  return a
...
def image_preprocessing(image_buffer, bbox, image_size, is_training):
  ...
  image1 = tf.py_func(my_func,[image],tf.double)
  image1=tf.image.convert_image_dtype(image1,tf.float32)
  image = tf.reshape(image1,[image_size, image_size, 3])
  return image
...
def imagenet_input(split, batch_size, image_size, is_training):
  ...
  if split.lower().startswith('train'):
    file_pattern = '/mnt/lustre/liukunlin/caad/alp/imagenet-tfrecord/train-*'
  elif split.lower().startswith('validation'):
    file_pattern = '/mnt/lustre/liukunlin/caad/alp/imagenet-tfrecord/validation-*'
  else:
